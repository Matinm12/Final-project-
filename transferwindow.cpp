...
void TransferWindow::onTransferClicked() {
    ...
    if (amount <= 100000) {
        ...
        if (amount > 100000) {
            // اگر هنوز رمز پویا ست نشده، تولیدش کن و نمایش بده
            if (from->getDynamicPassword2().empty()) {
                ...
                return;  // پایان موقت تا کاربر رمز رو بزنه در کلیک بعدی
            }

            // حالا که رمز تولید شده و کاربر وارد کرده، بررسیش کن
            if (pass2 != from->getDynamicPassword2()) {
                QMessageBox::warning(this, "خطا", "❌ رمز دوم پویا نادرست است.");
                return;
            }

            // پس از استفاده موفق، باطلش کن
