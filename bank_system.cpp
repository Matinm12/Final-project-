...
bool Customer::addAccount(BankAccount* acc) {
    if (accounts.size() >= 5) {
        return false; // بیش از ۵ حساب مجاز نیست
    }
...
BankAccount* Customer::findAccountByCard(const string& cardNum) {
    for (auto* acc : accounts)
...
void Customer::transferMoney() {
    ...
    // تاریخ انقضا بررسی: فرض فرمت "MM/YY"
    ...
    from->setDynamicPassword2(""); // رمز پویا فقط یک‌بار مصرفه
